  ...
  func executeCommand(_ command: String) async throws -> String {
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      sshHandlerFuture.whenSuccess { sshHandler in
        let createChannelFuture = sshHandler.createChannel(childChannelHandler) { childChannel, channelType in
          guard channelType == .session else {
            return channel.eventLoop.makeFailedFuture(SSHError.commandFailed)
          }
          return childChannel.eventLoop.makeSucceededFuture(())
        }
        
        createChannelFuture.whenFailure { error in
          promise.fail(error)
        }
      }
...
class ExecChannelHandler: ChannelInboundHandler {
  ...
  func channelActive(context: ChannelHandlerContext) {
    ...
    context.triggerUserOutboundEvent(SSHChannelRequestEvent.exec(execRequest), promise: nil)
  }
